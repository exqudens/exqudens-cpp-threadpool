...
void TestUtils::init(const std::vector<std::string>& args) {
    try {
        for (size_t i = 0; i < args.size(); i++) {
            if (!data.executableFile.has_value() && i == 0) {
                std::filesystem::path path = std::filesystem::path(args.at(i));
                data.executableFile = path.generic_string();
                if (!path.parent_path().empty()) {
                    data.executableDir = path.parent_path().generic_string();
                }
            }
            if (!data.projectBinaryDir.has_value() && i != 0 && args.at(i).starts_with("--project-binary-dir=")) {
                std::vector<std::string> parts = split(args.at(i), "=");
                if (parts.size() > 1) {
...
std::string TestUtils::toString(const std::pair<std::string, std::string>& value) {
    try {
...
std::string TestUtils::toString(const std::map<std::string, std::string>& value) {
    try {
        std::vector<std::string> values = {};
        for (const std::pair<std::string, std::string>& pair : value) {
            values.emplace_back(toString(pair));
        }
        return join(values, ", ", "{", "}");
    } catch (...) {
        std::throw_with_nested(std::runtime_error(CALL_INFO));
